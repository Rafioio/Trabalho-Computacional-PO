...
## Module: `src/model/function_normalizer.py`
...
A normalização é executada **automaticamente** durante a otimização (etapa 3/4):

```bash
# Executa otimização com normalização automática
python -m src.run
...
Para depuração, use `solve_from_file` com `verbose=True`:

```python
from src.model.solver import solve_from_file
results = solve_from_file("dados_generated.json", verbose=True)
```
...
| `src/model/function_normalizer.py` | New — `compute_payoff_table()` and `normalize_function()` |
| `src/run.py` | Normalização automática (etapa 3/4 do pipeline) |
